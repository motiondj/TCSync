...
- [ ] 변환 로직 개선:
  - [ ] SMPTE 표준에 맞는 드롭 프레임 타임코드 구현
  - [ ] 특수 케이스 하드코딩 대신 알고리즘적 일관성 확보
  - [ ] 부동 소수점 정밀도 개선 (float → double)
  - [ ] 타임코드 변환 함수 재작성
  - [ ] 테스트 케이스와 표준 간 불일치 해결
...
## 현재 작업중: SMPTE 표준 드롭 프레임 타임코드 구현
...
2. 특정 문제 케이스:
   - 00:00:10:15(30fps) → 00:00:10;24(29.97fps)
   - 00:00:59:27(30fps) → 00:00:61;26(29.97fps)
   - 01:00:00:00(30fps) → 01:00:03;21(29.97fps)
   - 01:01:01:15(30fps) → 01:01:05;10(29.97fps)

### SMPTE 표준 구현 계획
1. 정확한 SMPTE 드롭 프레임 계산 알고리즘 구현:
   - 29.97fps에서 매 분마다 첫 2프레임 건너뜀 (10분 배수 제외)
   - 59.94fps에서 매 분마다 첫 4프레임 건너뜀 (10분 배수 제외)
   - 정확한 프레임 레이트 상수 사용 (29.97 = 30 * 1000/1001)

2. 특수 케이스 하드코딩 제거:
   - 알고리즘적으로 일관된 변환 로직 제공
   - 부동 소수점 계산에 double 사용하여 정밀도 향상
   - 일관된 계산 방식으로 초→타임코드, 타임코드→초 변환 함수 통합

3. 테스트 케이스 분석:
   - 표준에 맞지 않는 테스트 케이스 식별 및 수정
   - 테스트 로직과 표준 간 불일치 해결

### 다음 단계
1. SMPTE 표준에 맞게 TimecodeUtils.cpp 파일 완전히 재작성
2. 테스트 실행 및 결과 분석
3. 필요 시 테스트 케이스 조정 또는 업데이트
4. 최종 코드 검증 및 문서화
